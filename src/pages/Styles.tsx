...
function PresetCard({
  ...
  const generateOnTemplate = async (tmpl: any) => {
    if (!user || busyTemplateId) return;
    setBusyTemplateId(tmpl.id);
    try {
      const res = await createWithStyle.mutateAsync({
        ...
        buildType: preset.build_type ?? null,
        rightsConfirmed: false,
      });
      // Hand off to the Concepts page; it owns the invocation so the
      // "Generating…" spinner reflects the in-flight request.
      setOpen(false);
      navigate(`/concepts?project=${res.project_id}`, { state: { autoGenerate: true } });
    } catch (e: any) {
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-border">
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button size="sm" variant="hero" className="flex-1" disabled={!user}>
              <Sparkles className="mr-1.5 h-3.5 w-3.5" /> Generate on a car
            </Button>
          </PopoverTrigger>
          <PopoverContent align="start" className="w-72 p-2 max-h-80 overflow-y-auto">
            <div className="text-mono text-[10px] uppercase tracking-widest text-muted-foreground px-2 py-1.5">
              Pick a car to apply <span className="text-foreground">{preset.name}</span>
            </div>
            {supportedTemplates.length === 0 ? (
              <div className="px-2 py-3 text-xs text-muted-foreground">No supported car templates.</div>
            ) : (
                ...
                {supportedTemplates.map((t: any) => {
                  ...
                  return (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => generateOnTemplate(t)}
                      disabled={!!busyTemplateId}
                      className={cn(
                        "flex items-center gap-2 w-full text-left rounded-md px-2 py-2 text-sm transition-colors",
                        busy ? "bg-primary/10 text-primary" : "hover:bg-surface-2 text-foreground",
                        busyTemplateId && !busy && "opacity-50",
                      )}
                    >
                      {busy ? (
                        <RefreshCw className="h-3.5 w-3.5 animate-spin text-primary shrink-0" />
                      ) : (
                        <CarIcon className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                      )}
                      <div className="min-w-0 flex-1">
